...
def parse_total_time(result_dir):
    ...
    if not file:
        return 0.0
    row = read_csv_row(file, 1)
    return float(row[1]) if row[1] else 0.0
...
def generate_comparison(results_dir):
    ...
    with open(out_path, "w", newline='') as outf:
        ...
        for mode in modes:
            ...
            uniq_cnt, _ = parse_unique_bugs(path)
            ...
            bug_types = parse_bug_type_summary(path)
            replay_written, replay_successful = parse_replay_stats(path)

            bpr = round((total_bugs / total_runs) * 1000, 2) if total_runs else 0
            bpm = round(total_bugs / (total_time / 60), 2) if total_time else 0
            rpm = round(total_runs / (total_time / 60), 2) if total_time else 0

            writer.writerow([
                mode, uniq_cnt, bug_types, total_bugs,
                panics, leaks, replays,
                total_runs, f"{total_time:.5f}", rec, ana, rep,
                bpr, bpm, rpm,
